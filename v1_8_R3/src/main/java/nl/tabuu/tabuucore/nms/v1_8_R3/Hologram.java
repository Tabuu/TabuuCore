...
public class Hologram implements IHologram {
    ...
    private void update(EntityArmorStand stand, HologramLine line, Player player) {
        ...
        if (line instanceof HologramItemLine) {
            ...
            stand.setEquipment(EnumItemSlot.HEAD.ordinal(), CraftItemStack.asNMSCopy(item.getItem()));
            stand.setCustomName(" ");
        } else {
            stand.setEquipment(EnumItemSlot.HEAD.ordinal(), CraftItemStack.asNMSCopy(XMaterial.AIR.parseItem()));
        }
        ...
        if(line instanceof HologramItemLine) {
            ...
            sendPacket(player, new PacketPlayOutEntityEquipment(stand.getId(), EnumItemSlot.HEAD.ordinal(), item));
        }
    }
