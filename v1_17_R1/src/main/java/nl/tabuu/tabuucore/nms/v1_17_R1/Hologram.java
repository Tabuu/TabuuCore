...
public class Hologram implements IHologram {
    ...
    private void update(EntityArmorStand stand, HologramLine line, Player player) {
        ...
        stand.aY = line.getLocation().getPitch();
        stand.aX = getLocation().getYaw();
        ...
        if (line instanceof HologramItemLine) {
            ...
            stand.setSlot(EnumItemSlot.f, CraftItemStack.asNMSCopy(item.getItem()));
        } else {
            stand.setSlot(EnumItemSlot.f, CraftItemStack.asNMSCopy(XMaterial.AIR.parseItem()));
        }
