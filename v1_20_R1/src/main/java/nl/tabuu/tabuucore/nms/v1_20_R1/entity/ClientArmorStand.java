package nl.tabuu.tabuucore.nms.v1_20_R1.entity;
...
public class ClientArmorStand implements IClientArmorStand {
    ...
    public Location getLocation() {
        org.bukkit.World world = Bukkit.getWorld(getHandle().cH().toString());
        Vec3D position = getHandle().dh();
        double x = position.a();
        double y = position.b();
        double z = position.c();
    ...
    public void sendPacketDestroy(Player player) {
        sendPacket(player, new PacketPlayOutEntityDestroy(getHandle().af()));
    }

    @Override
    public void sendPacketSpawn(Player player) {
        sendPacket(player, new PacketPlayOutSpawnEntity(getHandle()));
    }

    @Override
    public void sendPacketMetaData(Player player) {
        sendPacket(player, new PacketPlayOutEntityMetadata(getHandle().af(), getHandle().aj().b()));
    }
    ...
    public void sendPacketEquipment(Player player) {
        ...
        sendPacket(player, new PacketPlayOutEntityEquipment(_handle.af(), equipment));
    }
