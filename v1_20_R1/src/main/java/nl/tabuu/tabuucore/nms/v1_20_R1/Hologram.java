package nl.tabuu.tabuucore.nms.v1_20_R1;
...
public class Hologram implements IHologram {
    ...
    private void update(EntityArmorStand stand, HologramLine line, Player player) {
        sendPacket(player, new PacketPlayOutEntityDestroy(stand.af()));
        if(!isVisible()) return;

        stand.aX = line.getLocation().getPitch();
        stand.aW = getLocation().getYaw();
        ...
        sendPacket(player, new PacketPlayOutEntityMetadata(stand.af(), stand.aj().b()));
        ...
        if(line instanceof HologramItemLine) {
            ...
            sendPacket(player, new PacketPlayOutEntityEquipment(stand.af(), equipment));
        }
    }
    ...
    private void destroyArmorStand(EntityArmorStand stand) {
        ...
        sendPacket(new PacketPlayOutEntityDestroy(stand.af()));
        stand.ag();
    }
